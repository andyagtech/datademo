...
| Report Pal (Voice) | ✅ **Deployed** | Lambda for ephemeral Realtime API tokens + WebRTC voice |
...
- Ephemeral tokens generated by a dedicated Lambda (`backend/index.js`) — no API keys reach the browser
...
Browser (chat-widget.js)
  ...
  └── Voice: ephemeral token → WebRTC → OpenAI Realtime API (full-duplex audio)
                    ↑
        Session Lambda (backend/index.js) creates ephemeral tokens
```
...
│       └── step6_report.py
│
├── backend/                  # Voice session Lambda (Node.js)
│   ├── index.js              # Ephemeral OpenAI Realtime API token handler
│   └── package.json          # Dependencies + deploy scripts
│
├── cloud/                    # Chat Lambda (Python) — DuckDB + OpenAI
...
│   └── trust-policy.json     # IAM trust policy for voice session Lambda
│
